...
export const makeDefaultSubmissionService = (config) => {
    ...
    const submit = (transaction, waitForStatus = 'InBlock') => {
        return pipe(NodeClient.sendMidnightTransactionAndWait(transaction.serialize(), waitForStatus), Effect.provideServiceEffect(NodeClient.NodeClient, pipe(scopeAndClientDeferred, Deferred.await, Effect.map(({ client }) => client))), Effect.mapError((err) => WalletError.submission(err)));
    };
    return {
