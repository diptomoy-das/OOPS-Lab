...
import { Contract, ledger as veterinaryLedger } from './managed/veterinary/contract/index.js';
...
// Types
// ---------------------------------------------------------------------------
type VeterinaryPrivateState = { };
type VeterinaryCircuits = ImpureCircuitId<Contract<VeterinaryPrivateState>>;
const VeterinaryPrivateStateId = 'veterinaryPrivateState' as const;
...
async function main() {
  ...
  const zkConfigProvider = new NodeZkConfigProvider<VeterinaryCircuits>(zkConfigPath);

  const providers = {
    privateStateProvider: levelPrivateStateProvider<typeof VeterinaryPrivateStateId>({
  ...
  // --- Compile contract ---
  const veterinaryCompiledContract = CompiledContract.make('veterinary', Contract).pipe(
