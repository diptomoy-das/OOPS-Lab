/**
 * Standalone deploy script for the veterinary contract.
 *
 * Usage:
 *   npm run build && npm run deploy
 *
 * Prompts for a hex seed (or generates a new one), builds a wallet,
 * waits for sync + funds + dust, deploys the veterinary contract,
...
import { Contract } from './managed/veterinary/contract/index.js';
...
const zkConfigPath = path.resolve(currentDir, '..', 'src', 'managed', 'veterinary');
const deploymentPath = path.resolve(currentDir, '..', 'deployment.json');
// ---------------------------------------------------------------------------
// Logger
// ---------------------------------------------------------------------------
const logger = pino({ level: process.env.DEBUG_LEVEL ?? 'info' }, pinoPretty({ colorize: true, sync: true, translateTime: true, ignore: 'pid,time', singleLine: false }));
const VeterinaryPrivateStateId = 'veterinaryPrivateState';
// ---------------------------------------------------------------------------
...
const formatBalance = (b) => b.toLocaleString();
// ---------------------------------------------------------------------------
// Wallet build
// ---------------------------------------------------------------------------
...
async function registerForDustGeneration(wallet, unshieldedKeystore) {
    ...
    await withStatus('Waiting for dust tokens to generate', () => Rx.firstValueFrom(wallet.state().pipe(Rx.throttleTime(5_000), Rx.filter((s) => s.isSynced), Rx.filter((s) => s.dust.walletBalance(new Date()) > 0n))));
}
// ---------------------------------------------------------------------------
// Main
// ---------------------------------------------------------------------------
async function main() {
    ...
    console.log('\n╔══════════════════════════════════════════════════════════╗');
    console.log('║           Veterinary Contract Deploy Script             ║');
    console.log(`║           Network: ${NETWORK_ID.padEnd(38)}║`);
    console.log('╚══════════════════════════════════════════════════════════╝\n');
    // --- Seed prompt ---
    const rl = readline.createInterface({ input, output });
    const seedInput = await rl.question('Enter hex seed (leave blank to generate new): ');
    rl.close();
    let seed;
    if (seedInput.trim()) {
        seed = seedInput.trim();
    ...
    // --- Build wallet ---
    const { wallet, shieldedSecretKeys, dustSecretKey, unshieldedKeystore } = await withStatus('Building wallet', () => buildWallet(seed));
    ...
    // --- Wait for sync ---
    const syncedState = await withStatus('Syncing wallet with network', () => Rx.firstValueFrom(wallet.state().pipe(Rx.throttleTime(5_000), Rx.tap((s) => logger.debug(`Sync status: ${s.isSynced}`)), Rx.filter((s) => s.isSynced))));
    // --- Wait for funds ---
    ...
    if (balance === 0n) {
        await withStatus('Waiting for incoming funds (send tNight to address above)', () => Rx.firstValueFrom(wallet.state().pipe(Rx.throttleTime(10_000), Rx.tap((s) => {
            const u = s.unshielded?.balances[ledger.nativeToken().raw] ?? 0n;
            logger.debug(`Balance: ${u}`);
        }), Rx.filter((s) => s.isSynced), Rx.map((s) => (s.unshielded?.balances[ledger.nativeToken().raw] ?? 0n) +
            (s.shielded?.balances[ledger.nativeToken().raw] ?? 0n)), Rx.filter((b) => b > 0n))));
    }
    else {
    ...
    const providers = {
        privateStateProvider: levelPrivateStateProvider({
            privateStateStoreName: 'veterinary-private-state',
    ...
    // --- Compile contract ---
    const veterinaryCompiledContract = CompiledContract.make('veterinary', Contract).pipe(CompiledContract.withVacantWitnesses, CompiledContract.withCompiledFileAssets(zkConfigPath));
    // --- Deploy ---
    const contract = await withStatus('Deploying veterinary contract (this may take a few minutes)', async () => {
        return deployContract(providers, {
            compiledContract: veterinaryCompiledContract,
            privateStateId: VeterinaryPrivateStateId,
            initialPrivateState: {},
        });
    });
